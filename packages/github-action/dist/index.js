...
var pythonAdapter = {
  ...
  async detect(repoPath) {
    ...
    const checks = await Promise.all(markers.map((marker) => exists2(join2(repoPath, marker))));
...
import { access as access3, readFile, writeFile } from "fs/promises";
...
var PatchProofConfigSchema = external_exports.object({
  ...
  commands: external_exports.object({
    ...
    test: external_exports.object({
      run: external_exports.string().trim().min(1),
      timeout_ms: external_exports.number().int().positive().max(36e5).default(12e4),
  ...
  risk: external_exports.object({
    ...
    public_api_files: external_exports.array(external_exports.string().trim().min(1)).default([])
  }).strict().default({}),
  codex: external_exports.object({
    enabled: external_exports.boolean().default(false),
    max_context_bytes: external_exports.number().int().min(1024).max(2e5).default(32e3)
  }).strict().default({})
}).strict();
...
var CommandEvidenceSchema = external_exports.object({
  ...
  expected_exit_code: external_exports.number().int().min(0).max(255),
  exit_code: external_exports.number().int().min(0).max(255).nullable(),
...
var ProofSchema = external_exports.object({
  ...
  adapters: external_exports.array(external_exports.string()),
  commands: external_exports.object({
    reproduction: external_exports.object({
  ...
  changed_files: external_exports.object({
    all: external_exports.array(external_exports.string()),
    dependency: external_exports.array(external_exports.string()),
    public_api: external_exports.array(external_exports.string())
  }).strict(),
  determinations: external_exports.object({
    reproduced_on_base: external_exports.boolean(),
  ...
  verdict: external_exports.object({
    ...
    exit_code: external_exports.union([external_exports.literal(0), external_exports.literal(1)])
  }).strict(),
  codex: external_exports.object({
    enabled: external_exports.boolean(),
    verdict_influence: external_exports.literal("none")
  }).strict()
}).strict();
...
// ../core/src/report.ts
function renderJsonReport(proof) {
  return `${JSON.stringify(validateProof(proof), null, 2)}
`;
}
function renderMarkdownReport(proof) {
...
function commandSummary(label, command) {
  return [
    `### ${label}`,
    "",
...
// ../core/src/risk.ts
async function collectRiskPatterns(repoPath, adapters, configuredDependencyPatterns, configuredPublicApiPatterns) {
...
function matchChangedFiles(files, patterns) {
  return files.filter(
    (file) => patterns.some(
      (pattern) => file === pattern || minimatch(file, pattern, {
        dot: true,
  ...
  return await new Promise((resolve3) => {
    ...
    child.on("close", (exitCode, signal) => {
      ...
      resolve3({
        ...
        durationMs: Number((finished - started) / 1000000n),
        timedOut
      });
    });
  });
...
async function createWorktree(repoPath, worktreeRoot, label, ref, sha) {
  const path2 = join4(worktreeRoot, `${label}-${randomUUID()}`);
  await mkdir(dirname(path2), { recursive: true });
  await execFileAsync("git", ["worktree", "add", "--detach", path2, sha], {
    cwd: repoPath,
    maxBuffer: 10 * 1024 * 1024
  });
  return { path: path2, ref, sha };
}
...
async function verifyPatchProof(options) {
  ...
  const proofDir = options.proofDir ?? join5(repoPath, ".patchproof");
  const worktreeRoot = join5(proofDir, "worktrees", `${Date.now()}-${process.pid}`);
  let baseWorktree;
  let headWorktree;
  try {
    ...
    const proof = validateProof({
      ...
      config_path: resolve2(options.configPath),
      environment: {
...
async function writeProofFiles(proof, proofDir) {
  await mkdir2(proofDir, { recursive: true });
  const proofJsonPath = join5(proofDir, "proof.json");
  const proofMarkdownPath = join5(proofDir, "proof.md");
  await writeFile2(proofJsonPath, renderJsonReport(proof), "utf8");
  await writeFile2(proofMarkdownPath, renderMarkdownReport(proof), "utf8");
...
function inputEnv(inputName, fallbackName) {
