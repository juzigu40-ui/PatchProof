...
var nodeAdapter = {
  ...
  async detect(repoPath) {
    return await exists((0, import_node_path.join)(repoPath, "package.json"));
  },
...
async function exists2(path2) {
  try {
    await (0, import_promises2.access)(path2);
    return true;
  } catch {
    return false;
  }
...
var pythonAdapter = {
  ...
  async detect(repoPath) {
    ...
    const checks = await Promise.all(markers.map((marker) => exists2((0, import_node_path2.join)(repoPath, marker))));
...
async function removeWorktree(repoPath, worktreePath) {
  try {
    ...
    await (0, import_promises4.rm)(worktreePath, { force: true, recursive: true });
  }
}
...
async function writeProofFiles(proof, proofDir) {
  ...
  const proofJsonPath = (0, import_node_path5.join)(proofDir, "proof.json");
  const proofMarkdownPath = (0, import_node_path5.join)(proofDir, "proof.md");
  await (0, import_promises5.writeFile)(proofJsonPath, renderJsonReport(proof), "utf8");
